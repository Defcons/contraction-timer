...
- **`baby-tracker/index.html`** — vanilla JS, no build. Key symbols:
  ...
  - Log list renders newest-first capped at `logLimit` (50, "Show earlier" +100) with day separators; `renderHistory` shows 7-day totals (feeds / bottle ml / sleep / diapers); `csvBtn` exports full history to CSV, fetching server archives on demand.
  ...
  - `renderTimeline` — "Last 24 hours" strip, one lane per type (`TL_TYPES`, 8 lanes): bars for sleep/nurse (incl. live `active` timers), dots for the rest. Re-rendered by `render()` + a 15s interval.
  ...
  - Edit modal shows/hides field groups per type (`editEndWrap`, `editSideWrap`, `editAmountWrap`, `editMilkWrap`, `editKindWrap`, `editFoodWrap`, `editMedWrap`, `editHealthWrap`); pump reuses amount+side wraps.
